...
1. **Persona Spec** — A versioned JSON data file (`persona/spec/mirror_pete.v1.json`) that becomes the single source of truth for voice rules, banned phrases, platform adapters, safety boundaries, and signature motifs.
2. **Persona Renderer** — A two-pass compiler (`persona/renderer.js`) that applies deterministic transforms (banned phrase removal, structure enforcement, word replacements) then an LLM rewrite pass using the spec.
3. **Persona Validator** — A bouncer (`persona/validator.js`) that returns PASS/FAIL with hard-fail checks (deterministic) and scored checks (deterministic + optional LLM judge).
4. **Gate** — Wired into the Root App's Netlify function (`netlify/functions/generate.js`) so no output reaches the user without passing validation. Auto-retries up to 2 times with validator feedback injected.
...
```json
{
  ...
  "signature_motifs": {
    ...
    "roast_structures": [
      "Triple Escalation",
      "Compliment Execution",
      "Specific Callback"
    ],
    "openings": "(extracted from MirrorUniversePeteAgent pattern library — 6 entries)",
    "transitions": "(extracted from MirrorUniversePeteAgent pattern library — 5 entries)",
    "closings": "(extracted from MirrorUniversePeteAgent pattern library — 5 entries)"
  },
  ...
  "deterministic_transforms": {
    ...
    "cold_logic_prepends": "(4 entries from MirrorUniversePeteAgent)",
    "sarcasm_parentheticals": "(5 entries from MirrorUniversePeteAgent)"
  },

  "banned_patterns": {
    "phrases": [
      "leverage", "synergy", "align", "paradigm shift",
      "best practices", "thought leader", "circle back",
      "transform", "journey",
      "I'd be happy to help", "As an AI", "It's important to note"
    ],
    "regex_patterns": [
      "(?i)\\bleverage\\b",
      "(?i)\\bsynerg(y|ize|ies)\\b",
      "(?i)\\bthought leader(ship)?\\b"
    ]
  },
  ...
  "safety": {
    "hard_boundaries": [
      "No targeting individuals by name unless public figure in professional context",
      "No punching down",
      "No content that could constitute harassment"
    ],
    "content_warnings_required_for": [
...
**Actions:**
1. Extract exact values from all three sources into the spec
2. Resolve conflicts (e.g., satirical-pipeline bans "transform" but Root App doesn't — spec includes union of all bans)
3. Fill in the actual pattern library arrays from MirrorUniversePeteAgent (6 openings, 5 transitions, 5 closings — copy exact strings)
...
**Signature:**
```javascript
export async function render(inputText, platform, intent, spec, contextMeta = {}) → {
  renderedText, metadata
}
```
...
1. Remove all banned phrases/patterns (regex sweep using `spec.banned_patterns`)
2. Apply word replacements from `spec.deterministic_transforms.word_replacements`
3. Enforce platform structure skeleton from `spec.platform_adapters[platform].required_structure` (insert section markers if missing)
4. Trim to max_length if defined

**Pass 2 — LLM rewrite** (calls OpenAI via the same pattern as `generate.js`):
1. Build a system prompt from the spec: voice sliders, stance rules, platform adapter constraints, signature motifs
2. Send deterministic-pass output as user message with instruction to rewrite in persona voice
3. The LLM rewrite prompt is stored as a template string in `renderer.js` (not hardcoded in a prompt file) for co-location with the logic
...
**Signature:**
```javascript
export async function validate(candidateText, platform, spec, options = {}) → {
  pass, hardFailReasons, scores, suggestions
}
```

**Hard-fail checks** (deterministic, any failure = `pass: false`):
1. **Banned phrases**: Regex scan against `spec.banned_patterns.regex_patterns` — any match is a hard fail
2. **Structure compliance**: Check platform adapter's `required_structure` elements are present (heuristic: section headers, hooks, CTAs detected via keyword/pattern matching)
3. **Generic voice detection**: Check for `spec.banned_patterns.phrases` that indicate assistant-mode voice ("I'd be happy to help", "As an AI", "It's important to note")
4. **Safety boundary violation**: Check for `spec.safety.hard_boundaries` indicators (named individuals pattern, punching-down indicators)

**Scored checks** (0.0-1.0 scale, deterministic where possible):
1. **voice_fit**: Presence of word replacements from spec (count how many "Pete-isms" appear vs. generic equivalents)
2. **motif_presence**: Check if any joke formulas or roast structures are detectably used (keyword heuristics)
3. **structure_compliance**: Score completeness of required structure elements (partial credit)
4. **safety**: Inverse of safety-concern signals
5. **genericness**: Inverse score — count generic/corporate phrases, lower = better
...
```javascript
{
  ...
  suggestions: [
    "Consider adding a Technical Truth Bomb motif",
    "CTA is missing personality"
  ]
}
```
...
**Signature:**
```javascript
export async function gate(inputText, platform, intent, spec, options = {}) → {
  finalText, passed, attempts, validationResults, metadata
}
```

**Logic:**
1. Call `render(inputText, platform, intent, spec)`
2. Call `validate(renderedText, platform, spec)`
3. If passes → return `{ finalText: renderedText, passed: true, attempts: 1, ... }`
4. If fails and attempts < maxRetries (default 2):
   ...
   - Re-validate
5. After max retries exhausted → return `{ finalText: bestCandidate, passed: false, attempts: N, validationResults: [...all attempts...] }`
...
**What it does:**
1. Loads spec from `persona/spec/mirror_pete.v1.json`
2. Reads 5 fixture files from `persona/test/fixtures/`
3. For each fixture:
...
4. Runs `render()` in deterministic-only mode on the 3 generic fixtures
5. Validates rendered output — should improve scores
...
Phase 6: Test Harness & Fixtures
[ ] Write 2 passing fixtures (passing_linkedin.txt, passing_substack.txt)
[ ] Write 3 failing fixtures (generic_linkedin.txt, generic_substack.txt, too_mean_medium.txt)
